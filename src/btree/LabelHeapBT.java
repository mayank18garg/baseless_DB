 ...
 *
 */
public class LabelHeapBT  implements GlobalConst{
  ...
  protected final static byte[] getBytesFromEntry( KeyDataEntry entry ) 
	   ...
	   ConvertException
    {
	  ...
	  n+=4;
        else if (entry.data instanceof LabelHeapLeafData )
	  n+=8;
	  ...
	  Convert.setIntValue( ((LabelHeapLeafData)entry.data).getData().slotNo,
			       m, data);
	  Convert.setIntValue( ((LabelHeapLeafData)entry.data).getData().pageNo.pid,
			       m+4, data);
  ...
  public static void printPage(PageId pageno, int keyType)
    ...
    {
      LabelHeapBTSortedPage sortedPage=new LabelHeapBTSortedPage(pageno, keyType);
      int i;
      i=0;
      if ( sortedPage.getType()==NodeType.INDEX ) {
        LabelHeapBTIndexPage indexPage=new LabelHeapBTIndexPage((Page)sortedPage, keyType);
        System.out.println("");
	  ...
	  i++;    
        ...
        LabelHeapBTLeafPage leafPage=new LabelHeapBTLeafPage((Page)sortedPage, keyType);
        System.out.println("");
	  ...
	  if( keyType==AttrType.attrInteger) 
	    System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
			       (IntegerKey)entry.key+ ",  "+(LabelHeapLeafData)entry.data+ " )");
	  if( keyType==AttrType.attrString) 
	    System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
			       (StringKey)entry.key + ",  "+(LabelHeapLeafData)entry.data);
  ...
  private static void _printTree(PageId currentPageId, String prefix, int i, 
	   ...
	   ReplacerException 
    {
      ...
      LabelHeapBTSortedPage sortedPage=new LabelHeapBTSortedPage(currentPageId, keyType);
      prefix=prefix+"       ";
      i++;
      if( sortedPage.getType()==NodeType.INDEX) {  
	LabelHeapBTIndexPage indexPage=new LabelHeapBTIndexPage((Page)sortedPage, keyType);
   ...
   *@exception ReplacerException  error from lower layer
   */
  public static void printAllLeafPages(LabelHeapBTreeHeaderPage header)
    throws IOException, 
	   ConstructPageException, 
	   IteratorException,
	   HashEntryNotFoundException,
