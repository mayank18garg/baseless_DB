...
/** labelHeapbtfile.java
 * This is the main definition of class LabelBTreeFile, which derives from 
 * abstract base class LabelHeapIndexFile.
 * It provides an insert/delete interface.
 */
public class LabelHeapBTreeFile extends LabelHeapIndexFile
  ...
  private LabelHeapBTreeHeaderPage headerPage;
   ...
   *          of this btree file.
   */
  public LabelHeapBTreeHeaderPage getHeaderPage() {
    return headerPage;
  }
   ...
   */
  public LabelHeapBTreeFile(String filename)
    throws GetFileEntryException,  
	   PinPageException, 
    ...
    {      
      ...
      headerPage= new  LabelHeapBTreeHeaderPage( headerPageId);
      dbname = new String(filename);
      /*
       *
       * - headerPageId is the PageId of this LabelHeapBTreeFile's header page;
   ...
   */
  public LabelHeapBTreeFile(String filename, int keytype,
	...
	{
	  headerPage= new LabelHeapBTreeHeaderPage();
  ...
  private void  _destroyFile(PageId pageno) 
	   ...
	   FreePageException
    {
      ...
      LabelHeapBTSortedPage sortedPage;
      Page page=pinPage(pageno) ;
      sortedPage= new LabelHeapBTSortedPage( page, headerPage.get_keyType());
      
      if (sortedPage.getType() == NodeType.INDEX) {
	LabelHeapBTIndexPage indexPage= new LabelHeapBTIndexPage( page, headerPage.get_keyType());
	LID      lid=new LID();
	PageId       childId;
	KeyDataEntry entry;
  ...
  private void  updateHeader(PageId newRoot)
	     ...
	     UnpinPageException
    {
      ...
      LabelHeapBTreeHeaderPage header;
      PageId old_data;
      
      
      header= new LabelHeapBTreeHeaderPage( pinPage(headerPageId));
  ...
  public void insert(KeyClass key, LID lid) 
	   ...
	   
    {
      ...
      if (LabelHeapBT.getKeyLength(key) > headerPage.get_maxKeySize())
	...
	LabelHeapBTLeafPage newRootPage;
	LID dummylid;
	
	newRootPage=new LabelHeapBTLeafPage( headerPage.get_keyType());
	...
	{
	  LabelHeapBTIndexPage newRootPage;
	  PageId      newRootPageId;
	  Object      newEntryKey;
	  ...
	  newRootPage = new LabelHeapBTIndexPage(headerPage.get_keyType());
	  newRootPageId=newRootPage.getCurPage();
  ...
  private KeyDataEntry  _insert(KeyClass key, LID lid,  
	    ...
	    
    {
      ...
      LabelHeapBTSortedPage currentPage;
      Page page;
      KeyDataEntry upEntry;
      
      
      page=pinPage(currentPageId);
      currentPage=new LabelHeapBTSortedPage(page, headerPage.get_keyType());
	...
	LabelHeapBTIndexPage currentIndexPage=new LabelHeapBTIndexPage(page,
	...
	currentIndexPage= new LabelHeapBTIndexPage(pinPage(currentPageId),
					   headerPage.get_keyType() );
	...
	if (currentIndexPage.available_space() >=
	    LabelHeapBT.getKeyDataLength( upEntry.key, NodeType.INDEX))
	  {
	...
	if (LabelHeapBT.keyCompare( upEntry.key, tmpEntry.key) >=0 )
	  {
	  ...
	  tmpEntry =
	    LabelHeapBT.getEntryFromBytes(currentIndexPage.getpage(),
	...
	{
	  LabelHeapBTLeafPage currentLeafPage =
	    new LabelHeapBTLeafPage(page, headerPage.get_keyType() );
	  ...
	  if (currentLeafPage.available_space() >=
	      LabelHeapBT.getKeyDataLength(key, NodeType.LEAF) )
	    {
	      // no split has occurred
	  ...
	  if (rightPageId.pid != INVALID_PAGE)
	    {
	      LabelHeapBTLeafPage rightPage;
	      rightPage=new LabelHeapBTLeafPage(rightPageId, headerPage.get_keyType());
	  ...
	  for (tmpEntry = currentLeafPage.getFirst(firstLid);
	       ...
	       tmpEntry = currentLeafPage.getFirst(firstLid))
	    {
	      ...
	      newLeafPage.insertRecord( tmpEntry.key,
					((LabelHeapLeafData)(tmpEntry.data)).getData());
	    ...
	    if ( currentLeafPage.available_space() < 
		 newLeafPage.available_space()) {
	      newLeafPage.insertRecord( undoEntry.key, 
					((LabelHeapLeafData)undoEntry.data).getData());
  ...
  /* 
   * findRunStart.
   * Status LabelHeapBTreeFile::findRunStart (const void   lo_key,
   *                                LID          *pstartlid)
   *
  ...
  LabelHeapBTLeafPage findRunStart (KeyClass lo_key,
                                    LID startlid)
    throws IOException, 
	   IteratorException,  
	   KeyNotMatchException,
	   ...
	   UnpinPageException
    {
      LabelHeapBTLeafPage pageLeaf;
      LabelHeapBTIndexPage pageIndex;
      Page page;
      LabelHeapBTSortedPage sortPage;
      PageId pageno;
      ...
      page= pinPage(pageno);
      sortPage=new LabelHeapBTSortedPage(page, headerPage.get_keyType());
	...
	pageIndex=new LabelHeapBTIndexPage(page, headerPage.get_keyType());
	prevpageno = pageIndex.getPrevPage();
	curEntry= pageIndex.getFirst(startlid);
	while ( curEntry!=null && lo_key != null 
		&& LabelHeapBT.keyCompare(curEntry.key, lo_key) < 0) {
	...
	if ( trace!=null )
      ...
      pageLeaf = new LabelHeapBTLeafPage(page, headerPage.get_keyType() );
	...
	pageno = nextpageno; 
	pageLeaf=  new LabelHeapBTLeafPage( pinPage(pageno), headerPage.get_keyType());
	curEntry=pageLeaf.getFirst(startlid);
      }
	...
	// scan will unpin it when done
      ...
      while (LabelHeapBT.keyCompare(curEntry.key, lo_key) < 0) {
  ...
  private boolean NaiveDelete ( KeyClass key, LID lid)
      ...
      if ( trace!=null )
	{
	  ...
	  leafPage=new LabelHeapBTLeafPage(pinPage(nextpage),
				  headerPage.get_keyType() );
	  entry=leafPage.getFirst(new LID());
	}
	
	if ( LabelHeapBT.keyCompare(key, entry.key) > 0 )
	  break;
	...
	leafPage=new LabelHeapBTLeafPage(pinPage(nextpage), headerPage.get_keyType());
  ...
  private KeyClass _Delete ( KeyClass key,
      ...
      if ( trace!=null )
	{
      ...
      if (sortPage.getType()==NodeType.LEAF ) {
        ...
        PageId nextpage;
        LabelHeapBTLeafPage leafPage;
        leafPage=new LabelHeapBTLeafPage(page, headerPage.get_keyType());
	...
	while((tmpEntry!=null) && (LabelHeapBT.keyCompare(key,tmpEntry.key)>=0)) {
	  ...
	  if ( leafPage.delEntry(new KeyDataEntry(key, lid)) ) {
		...
		return null;
	      ...
	      // get a sibling
	      LabelHeapBTIndexPage parentPage;
	      parentPage =
		new LabelHeapBTIndexPage(pinPage(parentPageId),
				headerPage.get_keyType());
	      
	      PageId siblingPageId=new PageId();
	      LabelHeapBTLeafPage siblingPage;
	      int direction;
		...
		return null;
              ...
              siblingPage=new LabelHeapBTLeafPage(pinPage(siblingPageId),
					 headerPage.get_keyType());
		...
		LabelHeapBTLeafPage leftChild, rightChild;
		if (direction==-1) {
		...
		if ( rightChild.getNextPage().pid != INVALID_PAGE) {
		  LabelHeapBTLeafPage nextLeafPage=new LabelHeapBTLeafPage(
	...
	indexPage=new LabelHeapBTIndexPage(pinPage(currentPageId), headerPage.get_keyType());
	...
	if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
	  ...
	  if (indexPage.numberOfRecords() == 0) {
	    LabelHeapBTSortedPage childPage;
	    childPage=new LabelHeapBTSortedPage(indexPage.getPrevPage(),
				       headerPage.get_keyType()); 
	...
	if ((4 /*sizeof slot*/ +indexPage.available_space()) <= 
	  ...
	  // get a sibling
	  LabelHeapBTIndexPage parentPage;
	  parentPage=new LabelHeapBTIndexPage(pinPage(parentPageId),
				     headerPage.get_keyType()); 
	  
	  PageId siblingPageId=new PageId();
	  LabelHeapBTIndexPage siblingPage;
	  int direction;
	  ...
	  siblingPage=new LabelHeapBTIndexPage( pinPage(siblingPageId),
				       headerPage.get_keyType());
	  
	  int pushKeySize=0;
	  if (direction==1) {
	    pushKeySize=LabelHeapBT.getKeyLength
	  ...
	  if (siblingPage.redistribute(indexPage,parentPage, 
	    ...
	    // merge the two children
	    LabelHeapBTIndexPage leftChild, rightChild;
	    if (direction==-1) {
	      leftChild = siblingPage;
   ...
   *@exception UnpinPageException error when unpin a page
   */
  public LabelHeapBTFileScan new_scan(KeyClass lo_key, KeyClass hi_key)
    throws IOException,  
	   KeyNotMatchException, 
	   IteratorException, 
	   ConstructPageException, 
    ...
    {
      LabelHeapBTFileScan scan = new LabelHeapBTFileScan();
      if ( headerPage.get_rootId().pid==INVALID_PAGE) {
	scan.leafPage=null;
	return scan;
      }
  ...
  void trace_children(PageId id)
	...
	KeyClass key;
	KeyDataEntry entry;
	sortedPage=new LabelHeapBTSortedPage( pinPage( id), headerPage.get_keyType());
	
	
	// Now print all the child nodes of the page.  
	if( sortedPage.getType()==NodeType.INDEX) {
	  LabelHeapBTIndexPage indexPage=new LabelHeapBTIndexPage(sortedPage,headerPage.get_keyType());
	  ...
	  LabelHeapBTLeafPage leafPage=new LabelHeapBTLeafPage(sortedPage,headerPage.get_keyType());
