 ...
 */
public class LabelHeapBTIndexPage extends LabelHeapBTSortedPage {
   ...
   */
  public LabelHeapBTIndexPage(PageId pageno, int keyType)
    throws IOException, 
  ...
  /**associate the LabelHeapBTIndexPage instance with the Page instance,
   * also it sets the type of node to be NodeType.INDEX.
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public LabelHeapBTIndexPage(Page page, int keyType)
    throws IOException, 
	   ConstructPageException
    {
      super(page, keyType);
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public LabelHeapBTIndexPage( int keyType)
    throws IOException, 
	   ConstructPageException
    {
      super(keyType);
  ...
  LID deleteKey(KeyClass key) 
	...
	while ( LabelHeapBT.keyCompare(key, entry.key) > 0) {
	  entry = getNext(lid );
	  if (entry == null)
            break;
	}
	...
	else if (LabelHeapBT.keyCompare(key, entry.key) != 0)
  ...
  PageId getPageNoByKey(KeyClass key) 
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  entry= LabelHeapBT.getEntryFromBytes( getpage(),getSlotOffset(i),
  ...
  public KeyDataEntry getFirst(LID lid) 
	...
	entry= LabelHeapBT.getEntryFromBytes( getpage(),getSlotOffset(0),
				    getSlotLength(0),
  ...
  int  getSibling(KeyClass key, PageId pageNo)
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  ...
	  if (LabelHeapBT.keyCompare(key, entry.key)>=0) {
	    if (i != 0) { 
              entry=LabelHeapBT.getEntryFromBytes(getpage(), getSlotOffset(i-1),
   ...
   */
  boolean redistribute(LabelHeapBTIndexPage indexPage, LabelHeapBTIndexPage parentIndexPage,
	...
	if (direction==-1) { // 'this' is the left sibling of indexPage
	  ...
	  if ( (getSlotLength(0) + available_space()) > ((MAX_SPACE-DPFIXED)/2) ) {
            ...
            firstEntry= LabelHeapBT.getEntryFromBytes( getpage(),
					     getSlotOffset(0),
					     getSlotLength(0), keyType, NodeType.INDEX);
