...
public class LHFPage extends Page
  ...
  */
   ...
   * @param	labelBytes 	a record to be inserted
   * @return	LID of record, null if sufficient space does not exist
   * @exception IOException I/O errors
   * in C++ Status insertRecord(char *recPtr, int recLen, LID& lid)
   */
  public LID insertLRecord(byte[] labelBytes)
    throws IOException
    {
      LID lid = new LID();
      
      int recLen = labelBytes.length; // change this
	...
	System.arraycopy (labelBytes, 0, data, usedPtr, recLen);
	curPage.pid = Convert.getIntValue (CUR_PAGE, data);
   ...
   * @exception IOException I/O errors
   * in C++ Status deleteRecord(const LID& lid)
   */
  public void deleteLRecord(LID lid )
    throws IOException,  
   ...
   * @exception  IOException I/O errors
   * in C++ Status firstRecord(LID& firstRid)
   * 
   */ 
  public LID firstLRecord()
    throws IOException
    {
      LID lid = new LID();
   ...
   * @see    Tuple
   */
  public Label getLRecord(LID lid )
    throws IOException,  
	   InvalidSlotNumberException
    {
      short recLen;
   ...
   * <br>
   * in C++	Status returnRecord(LID lid, char*& recPtr, int& recLen)
   * @param       lid     the record ID
   ...
   * @see 	Tuple
   */  
  public Label returnLRecord(LID lid )
    throws IOException, 
	   InvalidSlotNumberException
    {
      short recLen;
	  ...
	  && (pageNo.pid == curPage.pid))
	{
	  ...
	  offset = getSlotOffset (slotNo);
      Label aLabel = new Label(data, offset, recLen);
	  return aLabel;
	}
