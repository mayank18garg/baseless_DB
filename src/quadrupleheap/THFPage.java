...
public class THFPage extends Page
  ...
  */
  ...
  /**
   * inserts a new record onto the page, returns QID of this record 
   * @param	record 	a record to be inserted
   * @return	QID of record, null if sufficient space does not exist
   * @exception IOException I/O errors
   * in C++ Status insertRecord(char *recPtr, int recLen, QID& qid)
   */
  public QID insertQRecord(byte [] record)
    throws IOException
    {
      QID qid = new QID();
  ...
  /**
   * delete the record with the specified qid
   * @param	qid 	the record ID
   * @exception InvalidSlotNumberException Invalid slot number
   * @exception IOException I/O errors
   * in C++ Status deleteRecord(const QID& qid)
   */
  public void deleteQRecord(QID qid )
    throws IOException,  
   ...
   * @exception  IOException I/O errors
   * in C++ Status firstRecord(QID& firstRid)
   * 
   */ 
  public QID firstQRecord()
    throws IOException
    {
      QID qid = new QID();
   ...
   * @exception  IOException I/O errors
   * in C++ Status nextRecord (QID curRid, QID& nextRid)
   */
  public QID nextQRecord(QID curQid)
    throws IOException 
    {
      QID qid = new QID();
      slotCnt = Convert.getShortValue (SLOT_CNT, data);
   ...
   * @see    Tuple
   */
  public Quadruple getQRecord(QID qid )
    throws IOException,  
	   InvalidSlotNumberException
    {
      short recLen;
  ...
  /**
   * returns a tuple in a byte array[pageSize] with given QID qid.
   * <br>
   * in C++	Status returnRecord(QID qid, char*& recPtr, int& recLen)
   * @param       qid     the record ID
   ...
   * @see 	Tuple
   */  
  public Quadruple returnQRecord(QID qid )
    throws IOException, 
	   InvalidSlotNumberException
    {
      short recLen;
	  ...
	  && (pageNo.pid == curPage.pid))
	{
	  ...
	  offset = getSlotOffset (slotNo);
        Quadruple tuple = new Quadruple(data, offset);
	  return tuple;
	}
